...
// AutoCompleterPlugin attempts to autocomplete using a plugins implementation
func AutoCompleterPlugin(b *Buffer) ([]string, []string) {
	if autocomp, ac := AutoComps[b.FileType()]; ac {
		plug := config.FindPlugin(autocomp.PlugName)
		ret, err := plug.Call(autocomp.FuncName, luar.New(ulua.L, b))
		if nil == err {
			completions_str := string(ret.(lua.LString))
			suggestions_str := string(ret.(lua.LString))

			completions := strings.Split(completions_str, "\n")
			suggestions := strings.Split(suggestions_str, "\n")
...
// TryBindAutoComplete attempts to add a new definition for an autocompletion plugin
func TryBindAutoComplete(plugnme string, lang string, funcnme string) bool {
	plug := config.FindPlugin(plugnme);
	if nil == plug {
		return false
	}
	if _, ac := AutoComps[lang]; !ac {
		AutoComps[lang] = AutoCompleter{plugnme, funcnme}
		return true
	}
